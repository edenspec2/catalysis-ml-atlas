...
test('corpus integrity and preserved atlas',async()=>{const g=JSON.parse(await readFile('dist/graph.json','utf8'));const ps=g.nodes.filter(p=>p.type==='paper');assert.equal(ps.length,55);assert.equal(new Set(ps.filter(p=>p.doi).map(p=>doiKey(p.doi))).size,54);assert.equal(ps.filter(p=>p.published_date).length,54);assert.equal(ps.filter(p=>p.chemistry_class&&p.representation_class).length,55);assert.ok(filterPapers(ps,{focus:'milo'}).length>=4);assert.ok(filterPapers(ps,{representation:'Learned 3D / TS GNN'}).length>=3);const ids=new Set(g.nodes.map(n=>n.id));for(const e of g.edges){assert.ok(ids.has(e.source));assert.ok(ids.has(e.target))}const h=await readFile('dist/atlas.html','utf8');assert.ok(!h.includes('https://unpkg.com'));assert.ok(h.includes('autoRotate=false'));assert.ok(h.includes('#side{display:grid'));assert.ok(h.includes('id="chemistry"'));assert.ok(h.includes('Color: representation'));new vm.Script(h.match(/<script>\s*([\s\S]*?)<\/script>/)[1])});
