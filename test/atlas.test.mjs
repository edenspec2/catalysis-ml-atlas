...
test('corpus integrity and preserved atlas',async()=>{const g=JSON.parse(await readFile('dist/graph.json','utf8'));const ps=g.nodes.filter(p=>p.type==='paper');assert.equal(ps.length,76);assert.equal(new Set(ps.filter(p=>p.doi).map(p=>doiKey(p.doi))).size,75);assert.ok(ps.filter(p=>p.published_date).length>=74);assert.ok(ps.every(p=>p.summary&&p.summary.length>24&&p.ask_next&&p.brief&&p.use_for&&p.stack_layer));
assert.ok(ps.filter(p=>p.figure?.src).length>=30);
assert.ok(g.insights?.stats?.papers===76);
...
test('reading UI: search, filter, details, save, read, compare and discovery failure',async()=>{
 ...
 const q=s=>w.document.querySelector(s);assert.equal(w.document.querySelectorAll('.paper').length,76);
 q('[data-view="inspect"]').click();
 assert.equal(w.document.querySelectorAll('table.inspect tbody tr').length,76);
 ...
 assert.equal(w.document.querySelectorAll('.paper').length,76);
 ...
 q('#close-detail').click();q('[data-view="board"]').click();assert.ok(q('.heat'));assert.ok(q('#results').textContent.includes('Open cells'));assert.ok(q('#results').textContent.includes('Stack layer'));
