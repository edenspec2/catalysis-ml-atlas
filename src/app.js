...
import {discover,filterPapers,doiKey} from './literature.js';
...
const state={view:'library',papers:[],feed:null,saved:load('saved',{}),read:load('read',{}),compare:[],active:null,loading:false,known:{}};
const find=id=>state.papers.find(p=>p.id===id)||state.feed?.papers.find(p=>p.id===id)||state.saved[id]||state.known[id];
...
function card(p){return '<article class="paper"><div class="paper-top"><span>'+esc(date(p))+'<br>'+esc(p.journal)+'</span><span class="badge">'+(p.candidate?(p.paper_type==='preprint'?'PREPRINT':'CANDIDATE'):'CURATED')+'</span></div><h3>'+button('data-detail',p.id,esc(p.full_title||p.label))+'</h3><p class="muted">'+esc(p.candidate?(p.derived_authors.slice(0,3).join(', ')+(p.derived_authors.length>3?' et al.':'')||'Author metadata unavailable'):p.why)+'</p><div class="tags">'+(p.candidate?['Crossref · not assessed']:[p.representation_class||p.paradigm,p.chemistry_class,...(p.groups||[]).filter(g=>g!=='Other')]).filter(Boolean).map(t=>'<span class="tag">'+esc(t)+'</span>').join('')+'</div><div class="actions">'+button('data-detail',p.id,'Details')+'<a href="'+esc(url(p.url))+'" target="_blank" rel="noopener">Paper ↗</a>'+button('data-save',p.id,state.saved[p.id]?'Saved ✓':'Save',!!state.saved[p.id])+button('data-compare',p.id,state.compare.includes(p.id)?'Comparing ✓':'Compare',state.compare.includes(p.id))+'</div>'+(state.read[p.id]?'<p class="read-status">Marked as read ✓</p>':'')+'</article>'}
function filters(){return{q:$('#search').value.trim(),focus:$('#focus').value,year:$('#year').value,unread:$('#unread').checked,read:state.read,chemistry:$('#chemistry')?.value||'',representation:$('#representation')?.value||''}}
function render(){
 ...
 const source=state.view==='latest'?(state.feed?.papers||[]):state.view==='saved'?Object.values(state.saved):state.papers;
 const ps=filterPapers(source,filters());
 $('#list-title').textContent={library:'Your research library',latest:'New in the literature',saved:'Your saved reading'}[state.view];$('#result-count').textContent=ps.length+' papers';
 $('#source-note').textContent=state.view==='library'?state.papers.length+' curated papers · Crossref dates and author lists checked 6 September 2026':state.view==='saved'?'Saved on this device. Reading state is not synced between browsers.':'Newest publication dates first within matched candidates. Title-keyword discovery can miss relevant work.';
 $('#results').innerHTML=ps.length?'<div class="cards">'+ps.map(card).join('')+'</div>':'<div class="empty"><h3>'+(state.loading?'Looking for recent papers…':'No papers in this view')+'</h3><p>'+(state.view==='saved'?'Save papers from the Library or Discover latest.':'Try a broader focus, clear the search, or increase the date window.')+'</p><button id="clear-filters">Clear filters</button></div>';
...
function details(id){
 ...
 const fields=p.candidate?[['Authors',p.derived_authors.join(', ')],['Record type',p.paper_type],['Research assessment','Not yet curated. Read the original paper to assess chemistry, data, validation and limitations.']]:[['Why it matters',p.why],['Chemistry class',p.chemistry_class],['Chemistry',p.chemistry],['Representation',p.representation_class],['Also in',(p.secondary_paradigms||[]).join(' · ')],['Data size',p.data_size_bin],['Data regime',p.data_regime],['Validation type',p.validation_type],['Validation',p.validation],['Methods / representation',(p.derived_methods||[]).join(' · ')],['Authors',(p.derived_authors||[]).join(', ')],['Research workflow',(p.derived_workflows||[]).join(' → ')],['Topics',(p.derived_topics||[]).join(' · ')]];
 $('#detail-body').innerHTML='<h2>'+esc(p.full_title||p.label)+'</h2><p class="muted">'+esc(date(p))+' · '+esc(p.journal)+'</p><div class="actions"><a class="primary" href="'+esc(url(p.url))+'" target="_blank" rel="noopener">Read original paper ↗</a>'+button('data-save',id,state.saved[id]?'Saved ✓':'Save paper')+button('data-read',id,state.read[id]?'Mark unread':'Mark as read')+button('data-compare',id,state.compare.includes(id)?'Remove comparison':'Add to compare')+'</div><dl>'+fields.filter(([,v])=>v).map(([k,v])=>'<dt>'+esc(k)+'</dt><dd>'+esc(v)+'</dd>').join('')+'<dt>DOI</dt><dd>'+esc(p.doi||'Not verified in the original corpus')+'</dd><dt>Provenance</dt><dd>'+(p.candidate?'Crossref publisher metadata. Candidate relevance uses title keywords; no scientific assessment has been added.':'Research interpretation comes from the v7 handoff, with Crossref years/authors overlaid. '+(p.metadata_source?'Title and publication date resolved through Crossref.':'Exact title and publication date have not been resolved.'))+'</dd></dl>'+(p.metadata_source?'<a href="'+esc(url(p.metadata_source))+'" target="_blank" rel="noopener">Metadata source ↗</a>':'');
 if(!$('#detail').open)$('#detail').showModal();$('#detail').scrollTop=0;
}
function compare(){
 const ps=state.compare.map(find).filter(Boolean);
 const fields=[['Published',p=>date(p)],['Research direction',p=>p.paradigm],['Also in',p=>(p.secondary_paradigms||[]).join(', ')],['Representation',p=>p.representation_class||p.derived_methods?.join(', ')],['Chemistry class',p=>p.chemistry_class],['Chemistry',p=>p.chemistry],['Data size',p=>p.data_size_bin],['Data regime',p=>p.data_regime],['Validation type',p=>p.validation_type],['Validation',p=>p.validation],['Why it matters',p=>p.why],['Workflow',p=>p.derived_workflows?.join(' → ')]];
...
function representations(){
 const rows=ORDER.map(rep=>{
  const ps=state.papers.filter(p=>p.representation_class===rep);
  if(!ps.length)return '';
